...
- `scan --fail-on-unscannable` - Treat files that could not be read (git-rendered binaries, permission errors) as a failure in `strict` mode; the installed pre-commit hook passes this flag so an unscannable staged file fails the commit instead of passing silently. `--update-baseline` writes a baseline instead of reporting, so the flag does not apply to that mode, and directories that cannot be listed are not counted
